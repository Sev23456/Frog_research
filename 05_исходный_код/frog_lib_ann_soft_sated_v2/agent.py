...
ANN_SOFT_STRIKE_SUPPRESSION = 0.47
ANN_SOFT_STRIKE_RELIEF = 0.08
ANN_SOFT_CAPTURE_REWARD_SUPPRESSION = 0.95
...
class FrogANNAgent:
    ...
    def _transition_reward(
        ...
        if self.prev_transition is not None:
            ...
            if strike_intent > 0.20 and reward_satiety_pressure > 0.0:
                total_reward -= (0.08 + 0.04 * current_brightness) * reward_satiety_pressure

        if reward > 0.0 and reward_satiety_pressure > 0.0:
            total_reward -= 0.62 * reward_satiety_pressure
    ...
    def _homeostatic_modulators(
        ...
        hunting_enabled = strike_permission >= 0.54 or hunger_drive >= 0.38
        return (
    ...
    def update(self, dt: float, flies: List[Any]) -> Dict[str, Any]:
        ...
        required_commitment = 2 if strike_permission >= 0.72 else 3
        required_strike_drive = 0.48 + 0.10 * satiety_gate
        required_strike_intent = -0.05 + 0.16 * satiety_gate
