...
class SNNFlyCatchingSimulation:
    def __init__(
        ...
        if not headless:
            ...
            pygame.display.set_caption("SNN Frog - spike-driven predator")
    ...
    def _neuro_panel_sections(self) -> List[Tuple[str, List[str]]]:
        ...
        return [
            ...
            (
                "Spike Dynamics",
                [
                    f"Spike count {state.get('spike_count', 0)} | rate {state.get('neural_activity', 0.0):.3f}",
                    f"Membrane {state.get('avg_membrane', 0.0):.3f} | refrac {state.get('refractory_fraction', 0.0):.2f}",
                    f"Eligibility {state.get('eligibility_norm', 0.0):.3f} | ctrl {state.get('controller_signal', 0.0):.2f}",
                ],
            ),
            (
                "Learning",
                [
                    f"Ext reward {state.get('reward', 0.0):.2f} | learn {state.get('learning_reward', 0.0):.3f}",
                    f"Juvenile progress {state.get('juvenile_progress', 0.0):.2f}",
                ],
            ),
            (
                "Sensory Focus",
                [
                    ...
                    f"Facing {state.get('focus_facing', 0.0):.2f}",
                ],
            ),
            (
                "Strike Loop",
    ...
    def _draw_hud(self):
        ...
        title = self.title_font.render("SNN Neuro Panel", True, YELLOW)
        self.screen.blit(title, (x, y))
        y += 26

        runtime_lines = [
            f"Flies alive {len([fly for fly in self.flies if fly.alive])}",
            f"Avg speed {np.mean(self.speed_history) if self.speed_history else 0.0:.2f} | avg spike {np.mean(self.spike_rate_history) if self.spike_rate_history else 0.0:.3f}",
            f"Silent {self.silent_steps / max(1, self.step_count):.2f} | burst {self.burst_steps / max(1, self.step_count):.2f}",
        ]
    ...
    def draw(self):
        ...
        pygame.draw.rect(self.screen, (230, 236, 246), (0, 0, self.width, self.height))
        for fly in self.flies:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "architecture_signature": "spike-driven predator decoder",
        }
