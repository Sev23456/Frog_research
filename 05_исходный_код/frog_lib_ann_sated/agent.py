...
ANN_SATIETY_STRIKE_STOP_RATIO = 0.80
ANN_SATIETY_STRIKE_RESUME_RATIO = 0.72
...
class FrogANNAgent:
    ...
    def __init__(
        ...
        self.satiety_strike_stop_ratio = ANN_SATIETY_STRIKE_STOP_RATIO
        self.satiety_strike_resume_ratio = ANN_SATIETY_STRIKE_RESUME_RATIO
    ...
    def _transition_reward(self, reward: float, focus_target: Optional[Dict[str, Any]]) -> float:
        ...
        total_reward = reward * 2.4 + approach * 0.60
    ...
    def _update_hunting_gate(self) -> Tuple[bool, float, float]:
        energy_ratio = clamp(self.energy / max(self.max_energy, 1e-6), 0.0, 1.0)
        if self.hunting_enabled:
            if energy_ratio >= self.satiety_strike_stop_ratio:
                self.hunting_enabled = False
        else:
            if energy_ratio <= self.satiety_strike_resume_ratio:
                self.hunting_enabled = True

        hunger_drive = clamp(
            (self.satiety_strike_stop_ratio - energy_ratio) / max(self.satiety_strike_stop_ratio, 1e-6),
            0.0,
            1.0,
        )
        satiety_gate = 1.0 - float(self.hunting_enabled)
        return bool(self.hunting_enabled), float(hunger_drive), float(satiety_gate)
    ...
    def update(self, dt: float, flies: List[Any]) -> Dict[str, Any]:
        ...
        if self.prev_transition is not None:
            learning_reward = self._transition_reward(reward, focus_target)
        ...
        hunting_enabled, hunger_drive, satiety_gate = self._update_hunting_gate()
