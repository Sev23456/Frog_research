...
def generate_report(output_dir: Path) -> Path:
    ...
    lines.append("- `ANN_SOFT_SATED_V2` and `ANN_FROZEN_SOFT_SATED_V2` are a stricter follow-up to the soft-sated branch: satiation dampens strikes earlier, reduces reward more strongly, and raises the strike commitment needed for catches while still avoiding a hard on/off gate.")
    lines.append("- `ANN_SOFT_SATED_V3` and `ANN_FROZEN_SOFT_SATED_V3` move satiety upstream into prey selection itself: satiation now down-weights weak prey before policy commitment, while very salient close prey can still break through.")
    lines.append("- `ANN_SOFT_SATED_V3_1` and `ANN_FROZEN_SOFT_SATED_V3_1` keep the upstream prey-selection gate from `V3`, but soften the thresholds so salient nearby prey can re-enter behavior before deep energy depletion.")
