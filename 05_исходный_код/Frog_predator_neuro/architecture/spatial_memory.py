...
from Frog_predator_neuro.core.biological_neuron import PyramidalNeuron
from Frog_predator_neuro.core.synapse_models import BiologicalSynapse
from Frog_predator_neuro.utils import clamp, normalize_vector
...
class SpatialMemory:
    ...
    def memory_vector(
        ...
        for unit, openness in candidates:
            ...
            # FIXED: Boost momentum influence (was 0.10×0.78=0.078, now 0.35×0.78=0.273)
        ...
        if np.linalg.norm(normalized) > 1e-6:
            ...
            # Old: 0.20-0.42 (forgot direction in 4-5 frames) → causes circling
            # New: 0.60-0.82 (remembers direction 20+ frames) → long-term path commitment
