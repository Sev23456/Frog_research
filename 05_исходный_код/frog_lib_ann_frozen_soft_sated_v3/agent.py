...
ANN_SOFT_STRIKE_SUPPRESSION = 0.38
ANN_SOFT_STRIKE_RELIEF = 0.10
ANN_SOFT_CAPTURE_REWARD_SUPPRESSION = 0.82
ANN_V3_TARGET_SUPPRESSION = 0.56
ANN_V3_TARGET_URGENCY_RELIEF = 0.46
ANN_V3_GOAL_THRESHOLD_BASE = 0.12
ANN_V3_GOAL_THRESHOLD_SATIETY = 0.12
ANN_V3_CLOSE_OVERRIDE = 0.55
...
class FrogANNAgent:
    ...
    def _target_homeostatic_profile(
        ...
        homeostatic_visibility = clamp(
            0.18
            + sensory_salience * (0.24 + 0.64 * hunger_drive)
            + urgency * ANN_V3_TARGET_URGENCY_RELIEF
            - satiety_pressure * ANN_V3_TARGET_SUPPRESSION * (1.0 - 0.75 * urgency),
            0.0,
            1.0,
        )
        goal_drive = clamp(
            sensory_salience * (0.34 + 0.96 * hunger_drive)
            + urgency * (0.18 + 0.24 * hunger_drive)
            - satiety_pressure * 0.34 * (1.0 - urgency),
            0.0,
            1.40,
        )
        return {
    ...
    def _transition_reward(
        ...
        if self.prev_transition is not None:
            ...
            if strike_intent > 0.20 and reward_satiety_pressure > 0.0:
                total_reward -= (0.04 + 0.03 * max(current_goal_drive, current_brightness)) * reward_satiety_pressure

        if reward > 0.0 and reward_satiety_pressure > 0.0:
            total_reward -= 0.30 * reward_satiety_pressure
    ...
    def _homeostatic_modulators(
        ...
        strike_permission = clamp(
            ...
            + 0.12 * goal_drive
            + 0.08 * homeostatic_visibility,
            0.38,
            1.0,
        )
        capture_reward_scale = clamp(self._capture_reward_scale(satiety_pressure) + 0.10 * goal_drive, 0.12, 1.0)
        hunting_enabled = goal_drive >= (0.22 + 0.16 * satiety_pressure) or hunger_drive >= 0.40 or closeness >= 0.82
        return (
    ...
    def update(self, dt: float, flies: List[Any]) -> Dict[str, Any]:
        ...
        if focus_target is not None:
            ...
            strike_signal *= clamp(0.32 + 0.74 * goal_drive, 0.18, 1.0)
        ...
        required_commitment = 2 if strike_permission >= 0.72 or goal_drive >= 0.72 else 3
        required_strike_drive = clamp(0.42 + 0.12 * satiety_gate - 0.08 * goal_drive, 0.28, 0.65)
        required_strike_intent = clamp(-0.08 + 0.16 * satiety_gate - 0.06 * goal_drive, -0.18, 0.20)
