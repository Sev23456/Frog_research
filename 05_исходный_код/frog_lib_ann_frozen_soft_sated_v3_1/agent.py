...
ANN_SOFT_SATIETY_PENALTY_START_RATIO = 0.45
ANN_SOFT_SATIETY_PENALTY_FULL_RATIO = 0.82
ANN_SOFT_STRIKE_SUPPRESSION = 0.34
ANN_SOFT_STRIKE_RELIEF = 0.12
ANN_SOFT_CAPTURE_REWARD_SUPPRESSION = 0.76
ANN_V3_TARGET_SUPPRESSION = 0.44
ANN_V3_TARGET_URGENCY_RELIEF = 0.56
ANN_V3_GOAL_THRESHOLD_BASE = 0.08
ANN_V3_GOAL_THRESHOLD_SATIETY = 0.10
ANN_V3_CLOSE_OVERRIDE = 0.42
...
class FrogANNAgent:
    ...
    def __init__(
        ...
        self.last_focus_goal_drive = 0.0
        self.last_homeostatic_visibility = 0.0
        self.last_focus_urgency = 0.0
    ...
    def _homeostatic_state(self) -> Tuple[float, float, float]:
        ...
        return float(energy_ratio), float(hunger_drive), float(satiety_pressure)

    def _target_homeostatic_profile(
        self,
        target: Dict[str, Any],
        hunger_drive: float,
        satiety_pressure: float,
    ) -> Dict[str, float]:
        distance_gain = clamp(1.0 - target["distance"] / max(1.0, self.visual_range), 0.0, 1.0)
        closeness = clamp(1.0 - target["distance"] / max(1e-6, self.catch_distance), 0.0, 1.0)
        sensory_salience = clamp(
            target["brightness"] * 0.54
            + target["motion"] * 0.18
            + max(0.0, target["facing"]) * 0.10
            + distance_gain * 0.18,
            0.0,
            1.0,
        )
        urgency = clamp(
            closeness * 0.58
            + target["brightness"] * 0.18
            + target["motion"] * 0.16
            + max(0.0, target["facing"]) * 0.08,
            0.0,
            1.0,
        )
        homeostatic_visibility = clamp(
            0.22
            + sensory_salience * (0.28 + 0.70 * hunger_drive)
            + urgency * ANN_V3_TARGET_URGENCY_RELIEF
            - satiety_pressure * ANN_V3_TARGET_SUPPRESSION * (1.0 - 0.78 * urgency),
            0.0,
            1.0,
        )
        goal_drive = clamp(
            sensory_salience * (0.42 + 0.90 * hunger_drive)
            + urgency * (0.24 + 0.30 * hunger_drive)
            - satiety_pressure * 0.24 * (1.0 - urgency),
            0.0,
            1.40,
        )
        return {
            ...
            "sensory_salience": float(sensory_salience),
            "urgency": float(urgency),
            "homeostatic_visibility": float(homeostatic_visibility),
            "goal_drive": float(goal_drive),
        }

    def _focus_target(self, hunger_drive: float, satiety_pressure: float) -> Optional[Dict[str, Any]]:
        if not self._visible_targets:
            return None
        best_target: Optional[Dict[str, Any]] = None
        best_score = -1.0
        for target in self._visible_targets:
            target.update(self._target_homeostatic_profile(target, hunger_drive, satiety_pressure))
            score = float(target["goal_drive"])
            if score > best_score:
                best_target = target
                best_score = score
        ...
        threshold = ANN_V3_GOAL_THRESHOLD_BASE + ANN_V3_GOAL_THRESHOLD_SATIETY * satiety_pressure
        if best_target["goal_drive"] < threshold and best_target["urgency"] < ANN_V3_CLOSE_OVERRIDE:
            return None
        return best_target
    ...
    def _observation(self, focus_target: Optional[Dict[str, Any]]) -> np.ndarray:
        ...
        if focus_target is not None:
            visibility_gate = clamp(focus_target.get("homeostatic_visibility", 1.0), 0.0, 1.0)
            goal_drive = clamp(focus_target.get("goal_drive", 0.0), 0.0, 1.0)
            distance_gain = clamp(1.0 - focus_target["distance"] / max(1.0, self.visual_range), 0.0, 1.0)
            direction = unit_vector(np.array(focus_target["vector"], dtype=float))
            obs[0] = float(direction[0] * visibility_gate)
            obs[1] = float(direction[1] * visibility_gate)
            obs[2] = float(distance_gain * visibility_gate)
            obs[3] = float(clamp(focus_target["brightness"], 0.0, 1.0) * visibility_gate)
            obs[4] = float(clamp(focus_target["motion"], 0.0, 1.0) * (0.55 + 0.45 * goal_drive))
    ...
    def _transition_reward(
        ...
        current_goal_drive = focus_target.get("goal_drive", 0.0) if focus_target is not None else 0.0
        current_homeostatic_visibility = focus_target.get("homeostatic_visibility", 0.0) if focus_target is not None else 0.0
        approach = clamp((self.last_focus_distance - current_distance) / max(1.0, self.visual_range), -0.45, 0.45)

        total_reward = reward * 2.2 * reward_scale + approach * 0.55
        total_reward += current_brightness * 0.03 + current_motion * 0.02
        total_reward += current_goal_drive * 0.03 + current_homeostatic_visibility * 0.02
        ...
        if self.prev_transition is not None:
            ...
            if strike_intent > 0.20 and focus_target is not None and current_distance < self.catch_distance * 1.10:
                ...
                total_reward -= 0.03
            if strike_intent > 0.20 and reward_satiety_pressure > 0.0:
                total_reward -= (0.03 + 0.02 * max(current_goal_drive, current_brightness)) * reward_satiety_pressure

        if reward > 0.0 and reward_satiety_pressure > 0.0:
            total_reward -= 0.22 * reward_satiety_pressure
    ...
    def _capture_reward_scale(self, satiety_pressure: float) -> float:
        return clamp(1.0 - ANN_SOFT_CAPTURE_REWARD_SUPPRESSION * satiety_pressure, 0.08, 1.0)

    def _homeostatic_modulators(
        self,
        focus_target: Optional[Dict[str, Any]],
    ) -> Tuple[bool, float, float, float, float]:
        _, hunger_drive, satiety_pressure = self._homeostatic_state()
        closeness = focus_target.get("closeness", 0.0) if focus_target is not None else 0.0
        goal_drive = focus_target.get("goal_drive", 0.0) if focus_target is not None else 0.0
        homeostatic_visibility = focus_target.get("homeostatic_visibility", 0.0) if focus_target is not None else 0.0
        strike_permission = clamp(
            0.78
            - ANN_SOFT_STRIKE_SUPPRESSION * satiety_pressure
            + ANN_SOFT_STRIKE_RELIEF * closeness
            + 0.14 * goal_drive
            + 0.10 * homeostatic_visibility,
            0.42,
            1.0,
        )
        capture_reward_scale = clamp(self._capture_reward_scale(satiety_pressure) + 0.12 * goal_drive, 0.16, 1.0)
        hunting_enabled = goal_drive >= (0.16 + 0.12 * satiety_pressure) or hunger_drive >= 0.38 or closeness >= 0.72
        return (
    ...
    def update(self, dt: float, flies: List[Any]) -> Dict[str, Any]:
        ...
        _, pre_hunger_drive, pre_satiety_pressure = self._homeostatic_state()
        focus_target = self._focus_target(pre_hunger_drive, pre_satiety_pressure)
        obs = self._observation(focus_target)
        reward_scale = self._capture_reward_scale(pre_satiety_pressure)

        if self.prev_transition is not None and self.learning_enabled:
            learning_reward = self._transition_reward(reward, focus_target, reward_scale, pre_satiety_pressure)
        ...
        goal_drive = focus_target.get("goal_drive", 0.0) if focus_target is not None else 0.0
        if focus_target is not None:
            closeness = focus_target.get("closeness", clamp(1.0 - focus_target["distance"] / max(1e-6, self.catch_distance), 0.0, 1.0))
            homeostatic_visibility = focus_target.get("homeostatic_visibility", 1.0)
            strike_signal = clamp(
                0.08
                + clamp((strike_intent + 1.0) * 0.5, 0.0, 1.0) * 0.56
                + closeness * 0.28
                + focus_target["brightness"] * 0.08 * homeostatic_visibility
                + max(0.0, alignment) * 0.06,
                0.0,
                1.0,
            )
            strike_signal *= clamp(0.40 + 0.70 * goal_drive, 0.25, 1.0)
        ...
        if not hunting_enabled:
            self.strike_drive *= 0.72
            self.strike_commitment = max(0, self.strike_commitment - 1)

        required_commitment = 2 if strike_permission >= 0.68 or goal_drive >= 0.62 else 3
        required_strike_drive = clamp(0.36 + 0.10 * satiety_gate - 0.10 * goal_drive, 0.24, 0.58)
        required_strike_intent = clamp(-0.10 + 0.12 * satiety_gate - 0.08 * goal_drive, -0.22, 0.16)
        ...
        ready_to_strike = (
            ...
            and goal_drive > 0.10
            and strike_intent > required_strike_intent
            and self.strike_drive > required_strike_drive
            and self.strike_commitment >= required_commitment
        ...
        self.last_focus_goal_drive = focus_target.get("goal_drive", 0.0) if focus_target is not None else 0.0
        self.last_homeostatic_visibility = focus_target.get("homeostatic_visibility", 0.0) if focus_target is not None else 0.0
        self.last_focus_urgency = focus_target.get("urgency", 0.0) if focus_target is not None else 0.0
        ...
        return {
            ...
            "visibility": float(focus_target.get("homeostatic_visibility", visibility) if focus_target is not None else visibility),
            "focus_brightness": float(self.last_focus_brightness),
            "focus_motion": float(self.last_focus_motion),
            "goal_drive": float(self.last_focus_goal_drive),
            "homeostatic_visibility": float(self.last_homeostatic_visibility),
            "focus_urgency": float(self.last_focus_urgency),
            "focus_sensory_salience": float(focus_target.get("sensory_salience", 0.0) if focus_target is not None else 0.0),
