...
class ANNFlyCatchingSimulation:
    def __init__(
        ...
        if not headless:
            ...
            pygame.display.set_caption("ANN Frozen Frog - soft homeostatic V3.1 predator")
        ...
        self.last_agent_state: Dict[str, Any] = {
            ...
            "focus_motion": 0.0,
            "goal_drive": 0.0,
            "homeostatic_visibility": 0.0,
            "focus_urgency": 0.0,
            "focus_sensory_salience": 0.0,
    ...
    def _neuro_panel_sections(self) -> List[Tuple[str, List[str]]]:
        ...
        return [
            ...
            (
                "Sensory Focus",
                [
                    f"Visibility {state.get('visibility', 0.0):.2f} | homeo vis {state.get('homeostatic_visibility', 0.0):.2f}",
                    f"Brightness {state.get('focus_brightness', 0.0):.2f} | motion {state.get('focus_motion', 0.0):.2f}",
                    f"Goal drive {state.get('goal_drive', 0.0):.2f} | urgency {state.get('focus_urgency', 0.0):.2f}",
                ],
            ),
            (
                "Homeostasis",
                [
                    f"Strike scale {state.get('strike_permission', 1.0):.2f} | satiety {state.get('satiety_gate', 0.0):.2f}",
                    f"Hunger drive {state.get('homeostatic_hunger_drive', 0.0):.2f} | reward scale {state.get('capture_reward_scale', 1.0):.2f}",
                    f"Salience {state.get('focus_sensory_salience', 0.0):.2f} | range {state.get('visual_range', 0.0):.0f}px",
                ],
            ),
            (
                "Strike Loop",
    ...
    def _draw_hud(self):
        ...
        title = self.title_font.render("ANN Frozen Soft-Homeostatic V3.1", True, YELLOW)
    ...
    def run_simulation(self, max_steps: int = 2000):
        print(f"ANN frozen homeostatic V3.1 simulation start: steps={max_steps}, headless={self.headless}, training={self.training_mode}")
        ...
        print(
            "ANN frozen homeostatic V3.1 summary | catches={caught_flies} success={success_rate:.3f} "
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "architecture_signature": "frozen actor-critic visual predator with softened upstream homeostatic prey-selection gate",
        }

    def save_state(self, filename: str = "ann_frozen_homeostatic_v3_1_frog_state.json"):
    ...
    def plot_results(self):
        ...
        fig.suptitle("ANN frozen homeostatic frog behavior summary")
