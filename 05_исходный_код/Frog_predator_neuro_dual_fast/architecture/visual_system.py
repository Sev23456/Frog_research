...
from Frog_predator_neuro_dual_fast.core.biological_neuron import LIFNeuron
...
class CenterSurroundFilter:
    ...
    def process(self, stimulus):
        dx = float(stimulus[0]) - float(self.position[0])
        dy = float(stimulus[1]) - float(self.position[1])
        distance_sq = dx * dx + dy * dy
        center_response = math.exp(-distance_sq / (2.0 * self.center_size ** 2))
        surround_response = math.exp(-distance_sq / (2.0 * self.surround_size ** 2))
        ...
        raw_drive = max(0.0, min(1.0, float(raw_drive)))
        motion_drive = abs(raw_drive - self.previous_drive)
        self.adaptation_trace = 0.92 * self.adaptation_trace + 0.08 * raw_drive
        habituated_drive = raw_drive * math.exp(-1.8 * self.adaptation_trace)
        combined_drive = max(0.0, min(1.0, 0.20 * habituated_drive + 0.80 * motion_drive))
    ...
    def process_visual_input(self, visual_scene):
        ...
        for idx, filter_cell in enumerate(self.filters):
            ...
            for obj_x, obj_y, brightness in visual_scene:
                total_response += filter_cell.process((obj_x, obj_y)) * brightness
